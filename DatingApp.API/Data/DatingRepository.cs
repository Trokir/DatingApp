...
namespace DatingApp.API.Data
{
    public class DatingRepository : IDatingRepository
    {
        ...
        {
            var users = _context.Users.Include(p => p.Photos).OrderByDescending(l => l.Lastactive).AsQueryable();
        ...
        private async Task<IEnumerable<int>> GetUserLikes(int id, bool likers)
        {
            ...
            {
                return user.Likees.Where(u => u.LikeeId == id).Select(x => x.LikeeId);
            }
        }
            ...
            switch (messageParams.MessagesContainer)
            {
                case "Inbox":
                    messages = messages.Where(u => u.RecipientId ==
                     messageParams.UserId && u.RecipientDeleted ==false);
                    break;
                case "Outbox":
